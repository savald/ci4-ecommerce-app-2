  ...
  <!-- Register Modal -->
  <div x-show="regModal" x-cloak x-transition:enter="transition-opacity ease-linear duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition-opacity ease-linear duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="mx-auto fixed inset-0 flex items-center justify-center overflow-y-auto bg-black z-50 bg-opacity-25">
    <div @click.outside="regModal = false" class="p-4 w-full max-w-md h-full md:h-auto">
      <!-- Modal content -->
      <div class="bg-white rounded shadow-lg dark:bg-gray-700">
        <!-- Close button -->
        <div class="flex justify-end p-2">
