<!-- Sidebar -->
<div x-data="{ dashboardSidebar: true }">
  <div x-show="dashboardSidebar" @open-sidebar.window="dashboardSidebar = true" @keyup.escape.window="dashboardSidebar = false" x-cloak x-transition:enter="transition-opacity ease-linear duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition-opacity ease-linear duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black z-40 bg-opacity-25 sm:hidden"></div>

  <aside x-show="dashboardSidebar" x-cloak x-transition:enter="transition ease-in-out duration-300 transform" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in-out duration-300 transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full" class="fixed z-40 top-0 pb-3 px-3 w-56 flex flex-col justify-between h-screen overflow-auto border-r bg-white transition duration-300 sm:static sm:w-52 md:px-6 md:w-60 lg:w-64 xl:w-72">
    <div class="realtive">
      <!-- Close nav mobile -->
      <div @click="dashboardSidebar = false" class="absolute right-1.5 top-1.5 inline-flex items-center justify-center rounded-full focus:bg-gray-100 active:bg-gray-200 p-1 sm:hidden">
      ...
      <ul class="space-y-2 tracking-wide mt-5">
        <li>
          <a href="#" aria-label="dashboard" class="px-4 py-3 flex items-center space-x-4 text-sm group rounded-xl shadow-lg text-white bg-gradient-to-r from-indigo-500 to-purple-400 lg:text-base">
        ...
        <li>
          <a href="<?= site_url('user/products'); ?>" class="px-4 py-3 flex items-center space-x-4 text-sm text-gray-600 group lg:text-base">
